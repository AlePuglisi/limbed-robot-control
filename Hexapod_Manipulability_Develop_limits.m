...
limbs_place = [0.05, 0.225, 0.40];
limbs_angle = [pi/6, pi/2, pi/2+pi/3];
Limbs = [];
for i=1:N_limb/2
    Limbs = [Limbs; Limb];
end
...
z_motion = 0.0; 
T_base_in = T_base0;
[q_new, T_base] = translate_base(ROBOT, T_base_in,  q0, x_motion, y_motion, z_motion);
% Update graphics 
...
%% LIMB MANIPULABILITY ELLIPSOID
% Initialization 
limbs_mask = [1 1 1 1 1 1]; % Visualize limbs ellipsoid
