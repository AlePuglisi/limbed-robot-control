...
[q1,T_base] = translate_base(ROBOT,Limb_swing, T_base0, q, 0.1, 0.0, 0.0);
plot_robot(ROBOT, q);
...
%% Translate the base
function [q_new, T_base] = translate_base(ROBOT,new_limb, T_base_in,  q, x_motion, y_motion, z_motion)
    N_limb = length(ROBOT);
    ...
    for i = 1:N_limb
        if contacts(i) == 1 % only contacts limb move the base
            if version == 1 % moving base using contact model 
            ...
            elseif version == 2 % moving base using swing model 
                q_swing(i,:) = flip(q(i,:));
                ...
                q_swing(i,:) = new_limb.ikine(T_ee(:,:,i), q_swing(i,:), 'mask', [1 1 1 0 0 0]);
