...
limbero_contact.tool = transl(0.1, -0.1, 0);
limbero_contact.payload(3.1152/4 + limbero_contact.links(7).m, [0.17, -0.17, 0]);

%%  PLOT
figure('Name', 'LIMBERO LF LEG, CONTACT DH (Gripper-coxa)')
limbero_contact.plot([0, 0, 0, 0, 0, 0, 0], 'workspace', [-1 1 -1 1 -1 1], 'view', [30 30], 'scale', 0.6,'jaxes', 'nobase', 'noshadow', 'notiles');
...
n_angles = 50; % number of steps 
% initialize matrix collecting all combinations
q0 = zeros(N_link,n_angles);
q0_contact = zeros(N_link, n_angles);
% neglect driving DOF for this identification
q0(N_link,:) = zeros(1,n_angles);
q0_contact(1,:) = zeros(1,n_angles);

% generate a 7x100 matrix, each row is a DOF, and each column a fixed step
% % angle on that DOF

% for i=1:N_link-1
%     span = limbero.links(i).qlim(2)-limbero.links(i).qlim(1);
%     delta = span/(n_angles-1); 
%     q0(i,:) =  [limbero.links(i).qlim(1):delta:limbero.links(i).qlim(2)];
% 
%     span_contact = limbero_contact.links(i+1).qlim(2)-limbero_contact.links(i+1).qlim(1);
%     delta_contact = span_contact/(n_angles-1); 
%     q0_contact(i+1,:) =  [limbero_contact.links(i+1).qlim(1):delta_contact:limbero_contact.links(i+1).qlim(2)];
% end
...
% 2) use identifyed inertia to tune indipendent joint PID controller 
%% CONTROLLER TUNING 
w_cp = 20; %[rad/s] ~2 Hz
w_cv = 10*w_cp; 
...
for i=1:limbero.n
    KP(i) = kpv(i)*(kpp+1/Tiv);
    TD(i) = kpv(i)/KP(i);
    TI(i) = KP(i)*Tiv/(kpp*kpv(i));
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TRAJECTORY GENERATION for SIMULATION %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TRAJECTORY GENERATION at joint level
t0 = 0.5; 
t_tot = 2.5; 

q_displacement = zeros(1,7); 
q_displacement(2) = -0.7*pi/2;
q_displacement(3) = pi/4;

q_dot_max = ones(1,7)*pi;
q_dotdot = zeros(1,7);
t_acc = zeros(1,7);
for i=1:length(t_acc)
    t_acc(i) = t_tot - q_displacement(i)/q_dot_max(i);
    q_dotdot(i) = q_dot_max(i)/t_acc(i); 
end
...
% t_tot = 1.5; 
% 
% task_displacement = zeros(1,3); 
% task_displacement(1) =  - 0.1;  % dx
% task_displacement(3) = - 0.1; % dz
% 

% x_dot_max = ones(1,3); % 1 m/s
% x_dotdot = zeros(1,3);
% t_acc = zeros(1,3); 
% 
% for i=1:length(t_acc)
%     t_acc(i) = (t_tot*x_dot_max(i) - task_displacement(i))/x_dot_max(i);
%     x_dotdot(i) = x_dot_max(i)/t_acc(i); 
% end
